  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-8 gap-4">
        <div className="animate-spin w-8 h-8 border-2 border-dh-accent-text border-t-transparent rounded-full" />
        <p className="text-sm dh-text-secondary">{t('lab.loading')}</p>
      </div>
    );
  }
  ...
  if (status === 'unavailable') {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-8 gap-4">
        ...
        <h3 className="text-lg font-medium dh-text-primary">
          {t('lab.comingSoonTitle')}
        </h3>
        <p className="text-sm dh-text-secondary max-w-md">
          {t('lab.comingSoonDesc')}
        </p>
        <span className="inline-block px-3 py-1 text-xs font-medium dh-accent-text dh-accent-bg/30 rounded-full border dh-accent-border/40">
