...
export const PanelSeriesRail: React.FC<PanelSeriesRailProps> = ({
  ...
  if (series.length === 0) {
    return (
      <div className="w-44 shrink-0 border-r border-white/5 px-3 py-2 text-xs dh-text-secondary">
        {labels.emptyHint}
      </div>
    );
  }
        ...
        {series.map((s, i) => {
          ...
          return (
            <li
              key={key}
              className={[
                'px-2 py-1.5 border-b dh-border-default/40',
            ...
            >
              ...
              <div className="flex items-center gap-1.5 mb-1.5">
                <label className="relative inline-flex shrink-0 cursor-pointer" title={cfg.colorOverride ?? color}>
                  <span
                    aria-hidden
                    className="inline-block w-3 h-3 rounded-full ring-1 ring-transparent"
                ...
                <span
                  className="text-xs dh-text-primary truncate flex-1 min-w-0"
                  title={`${s.attribute} (${s.entityId})`}
                >
                  {s.attribute}
                </span>
                {unit && <span className="text-[10px] dh-text-secondary">{unit}</span>}
              </div>
              ...
              <div className="flex items-center gap-0.5">
                <button
                  ...
                  className={[
                    ...
                    (s.yAxis ?? 'left') === 'left'
                      ...
                      : 'dh-bg-surface border-white/10 dh-text-secondary hover:dh-bg-surface-alt',
                  ].join(' ')}
                ...
                <button
                  ...
                  className="px-1.5 py-1 border border-white/10 dh-bg-surface dh-text-secondary hover:dh-bg-surface-alt hover:dh-text-primary transition-colors"
                ...
                <button
                  type="button"
                  onClick={() => onRemove(i)}
                  className="px-1.5 py-1 rounded-r border border-white/10 dh-bg-surface dh-text-secondary hover:bg-rose-900/40 hover:text-rose-300 hover:border-rose-700 transition-colors"
                  title={labels.remove}
                >
...
const Stat: React.FC<{ label: string; value: number }> = ({ label, value }) => (
  <span className="flex items-center gap-1 dh-text-muted truncate">
    <span>{label}</span>
    <span className="dh-text-primary">{formatNumberShort(value)}</span>
  </span>
);
