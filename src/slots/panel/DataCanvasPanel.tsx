...
export const DataCanvasPanel: React.FC<DataCanvasPanelProps> = ({
  ...
  return (
    <div ref={rootRef} className="relative w-full h-full rounded-lg ring-1 ring-transparent dh-bg-main" onContextMenu={handleContextMenu}>
      ...
      {status === 'loading' && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span className="px-4 py-2 rounded-full dh-bg-surface-alt border dh-border-light dh-text-primary text-sm">
      ...
      <div className="absolute top-0 left-0 right-0 px-2 py-1.5">
        <div className="flex items-center gap-2">
          {/* Title + drag handle */}
          <div className="panel-drag-handle cursor-move dh-bg-surface-alt/95 backdrop-blur-sm rounded-lg pl-3 pr-3 py-2 border dh-border-light shadow-lg select-none">
            <span className="text-sm dh-text-primary font-mono font-medium truncate max-w-[300px] tracking-tight">
              {headerTitle}
            </span>
          </div>
          ...
          {status === 'ready' && (
            <button
              ...
              className={`px-3 py-2 text-sm rounded-lg transition-colors flex items-center gap-2 shadow-lg border ${
                toolbarOpen
                  ? 'dh-text-primary dh-bg-surface-alt dh-border-light'
                  : 'dh-text-primary dh-bg-surface-alt/95 dh-border-light hover:text-white hover:dh-bg-surface-alt hover:border-[#3b465b]'
              }`}
              title="Chart tools"
            >
              ...
              <span className="text-sm dh-text-secondary">Tools</span>
            </button>
          )}
        </div>
        {toolbarOpen && status === 'ready' && (
          <div className="pointer-events-auto px-2 pb-1"
            onMouseDown={(e) => e.stopPropagation()}
          >
            <div className="dh-bg-surface rounded-lg border dh-border-default shadow-2xl max-h-[80vh] overflow-y-auto">
              <PanelToolbar
                appearance={appearance}
              ...
              />
              {railOpen && (
                <div className="border-t dh-border-default p-2 dh-bg-surface">
                  <PanelSeriesRail
              ...
              <div className="border-t dh-border-default/50">
                <DerivedSeriesInput
