...
export const PanelTooltip: React.FC<PanelTooltipProps> = ({
  ...
  return (
    <div
      className="absolute z-30 pointer-events-none px-3 py-2.5 rounded-lg bg-slate-900 border border-slate-500 shadow-2xl min-w-[180px] max-w-[300px]"
      style={{
        left: left + offsetX,
    ...
    >
      <div className="text-xs text-slate-300 mb-2 font-mono tabular-nums">{timestamp}</div>
      <div className="flex flex-col gap-1.5">
        {rows.map((r, i) => (
          <div key={`${r.label}-${i}`} className="flex items-center gap-2 leading-tight text-sm">
            ...
            />
            <span className="text-slate-100 truncate" title={r.label}>
              {r.label}
            </span>
            <span className="ml-auto tabular-nums text-white font-semibold whitespace-nowrap">
              {formatNumberShort(r.value)}
              {r.unit && <span className="text-slate-300 font-normal ml-0.5">{r.unit}</span>}
            </span>
          </div>
        ))}
      </div>
