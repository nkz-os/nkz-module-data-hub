...
export const PanelTooltip: React.FC<PanelTooltipProps> = ({
  ...
  return (
    <div
      className="absolute z-30 pointer-events-none px-2.5 py-2 rounded-lg bg-background border border-border shadow-2xl min-w-[160px] max-w-[280px]"
      style={{
        left: left + offsetX,
    ...
    >
      <div className="text-[10px] text-muted-foreground mb-1.5 font-mono tabular-nums">{timestamp}</div>
      <div className="flex flex-col gap-1">
        {rows.map((r, i) => (
          <div key={`${r.label}-${i}`} className="flex items-center gap-2 leading-tight text-[11px]">
            ...
            <span className="ml-auto tabular-nums text-foreground font-semibold whitespace-nowrap">
              {formatNumberShort(r.value)}
              {r.unit && <span className="text-muted-foreground font-normal ml-0.5">{r.unit}</span>}
            </span>
          </div>
        ))}
      </div>
