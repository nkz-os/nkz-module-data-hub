    ...
    <div className="px-2 pb-2 pt-1 space-y-1.5">
      <div className="flex items-center gap-1.5">
        <input
          ...
          maxLength={60}
          className="flex-1 px-2 py-1 text-[10px] rounded border border-white/10 dh-bg-surface dh-text-primary placeholder-current opacity-50"
        />
        ...
        <button
          ...
          onClick={handleAdd}
          disabled={!label.trim()}
          className="p-1 rounded dh-text-secondary hover:dh-text-primary disabled:opacity-30"
        >
          <Plus size={12} />
        </button>
      </div>
        ...
        <ul className="space-y-0.5">
          {annotations.map((a) => (
            <li key={a.id} className="flex items-center gap-1.5 text-[10px]">
              ...
              <span className="dh-text-primary truncate flex-1 min-w-0">{a.label}</span>
              <span className="dh-text-muted tabular-nums font-mono">{fmtEpoch(a.xEpoch)}</span>
              <button
                type="button"
                onClick={() => onDelete(a.id)}
                className="p-0.5 dh-text-muted hover:text-red-400"
              >
                <Trash2 size={10} />
              </button>
            </li>
